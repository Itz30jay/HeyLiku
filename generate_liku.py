"""Helper script to generate liku_assistant files."""
import os

TARGET_DIR = r"C:\Users\jayki\liku_assistant"
...
# README.md content
# =====================================================================
README_MD = '''# Liku - Your Free Offline Voice Assistant for Windows

Liku is a completely **free, open-source voice assistant** that runs locally on your Windows PC. No API keys, no subscriptions, no cloud dependency (except for YouTube search).

## What Can Liku Do?

- **Wake word activation** - Say "Hey Liku" to get started
- **Open apps** - VS Code, Chrome, Notepad, Calculator, and many more
- **Open websites** - Google, YouTube, Gmail, GitHub, etc.
- **Play YouTube videos** - "Hey Liku, play magician video on YouTube"
- **Control volume** - Volume up, down, mute
- **Media controls** - Pause, resume, next track, previous track
- **Take screenshots** - Saved to your Pictures folder
- **Lock your PC** - "Hey Liku, lock the computer"
- **Google search** - "Hey Liku, search Google for Python tutorials"
- **Time & Date** - "Hey Liku, what time is it?"
- **AI Chat** - Ask any question and get a spoken answer (via Ollama)
...
```
cd C:\\Users\\jayki\\liku_assistant
...
2. Download: **vosk-model-small-en-in-0.4** (Indian English, ~36 MB)
3. Extract the ZIP file
4. **Rename** the extracted folder to just `model`
5. Place it inside `C:\\Users\\jayki\\liku_assistant\\` so the structure looks like:
   ```
   liku_assistant/
   ├── liku.py
   ├── requirements.txt
...
### Step 4: Install & Set Up Ollama (for AI Chat)
...
3. Open a terminal and pull the model:
   ...
   ```
4. Make sure Ollama is running (it runs in the background after installation)
...
## 15 Example Voice Commands

| # | Say This | What Happens |
|---|----------|--------------|
| 1 | "Hey Liku, open VS Code" | Opens Visual Studio Code |
| 2 | "Hey Liku, open Chrome" | Opens Google Chrome |
| 3 | "Hey Liku, open YouTube" | Opens youtube.com in your browser |
| 4 | "Hey Liku, play magician video on YouTube" | Searches & plays the video |
| 5 | "Hey Liku, turn on YouTube and play lofi music" | Searches & plays lofi music |
| 6 | "Hey Liku, open Google" | Opens google.com |
| 7 | "Hey Liku, search Google for weather today" | Opens a Google search |
| 8 | "Hey Liku, volume up" | Increases system volume |
| 9 | "Hey Liku, mute" | Toggles mute |
| 10 | "Hey Liku, take a screenshot" | Saves screenshot to Pictures |
| 11 | "Hey Liku, what time is it?" | Tells you the current time |
| 12 | "Hey Liku, what is today's date?" | Tells you today's date |
| 13 | "Hey Liku, lock the computer" | Locks your PC |
| 14 | "Hey Liku, close Notepad" | Closes Notepad |
| 15 | "Hey Liku, what is the capital of France?" | AI answers: "Paris!" |

**Tip:** You can also just say "Hey Liku" by itself. Liku will say "Yes?" and wait 8 seconds for your command.

---

## How to Add Your Own Apps

Open `liku.py` and find the `APPS` dictionary near the top. Add your app like this:
...
Then just say "Hey Liku, open Photoshop" and it will work!

---

## Cute Girl Voice Customization

Liku now speaks with an ultra-realistic, human-sounding cute voice instead of a robot!
Open `liku.py` and find the `Voice settings` in `CONFIGURATION` near the top:

```python
NEURAL_VOICE = "en-US-AnaNeural"  # Cute, sweet youthful voice
VOICE_PITCH = "+0Hz"               # Adjust pitch (+5Hz for extra cuteness)
VOICE_SPEED = "+0%"                # Adjust speed
```

Popular voice options:
- `"en-US-AnaNeural"` — Sweet, cute youthful girl voice (Default)
- `"en-US-AvaNeural"` — Cheerful, expressive 23-year-old young woman
- `"en-US-JennyNeural"` — Friendly, natural young female assistant
- `"en-IN-NeerjaExpressiveNeural"` — Expressive young Indian-English female

If you are offline, Liku automatically falls back to your local Windows female voice seamlessly.
...
| Problem | Solution |
|---------|----------|
| "ERROR: Vosk model not found!" | Download the model and rename the folder to `model` (see Step 3) |
| "I can't reach the Ollama server" | Make sure Ollama is running (ollama serve in a terminal) |
| "The model is not downloaded yet" | Run `ollama pull llama3.2:1b` in a terminal |
| Liku doesn't hear me | Check that your microphone is connected and not muted |
| Liku hears itself speaking | This should be handled automatically. If not, move the mic away from speakers |
| pyttsx3 hangs | The code creates a fresh engine each time to avoid this. If it still hangs, restart the script |
...
```
liku_assistant/
├── liku.py              <- Main assistant (run this!)
├── requirements.txt     <- Python packages to install
├── test_liku.py         <- Tests (no microphone needed)
...
This project is free and open source. Built for learning and personal use.
No paid APIs, no API keys, no subscriptions. Everything runs on your machine.
'''
...
# Write all files
# =====================================================================
TARGET_DIRS = [TARGET_DIR, os.path.dirname(os.path.abspath(__file__))]
...
print("Writing Liku assistant files...")
write_file("liku.py", LIKU_PY)
write_file("requirements.txt", REQUIREMENTS_TXT)
write_file("test_liku.py", TEST_PY)
